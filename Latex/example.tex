...
\newcommand{\thesistitle}{Performante Evaluierung von Well-Designed Pattern Trees mit Hilfe von In-Memory Datenbanken} % The title of the thesis. The English version should be used, if it exists.
...
\setfirstassistant{}{Wolfgang Fischl}{MSc}{male}
...
\settitle{\thesistitle}{Performante Evaluierung von Well-Designed Pattern Trees mit Hilfe von In-Memory Datenbanken} % Sets English and German version of the title (both can be English or German).
...
\chapter{Einleitung}
\section*{Motivation}Das Abfragen von Information ist heutzutage einer der wichtigsten Bestandteile unseres täglichen Lebens. Vieler dieser täglichen Abfragen verwenden im Hintergrund für den Benutzer nicht sichtbar, durch eine relationale Datenbank abgewickelt. Ähnlich dazu verwendet man im Semantic Web die Abfragesprache SPARQL. SPARQL steht für \textit{SPARQL Protocol And RDF Query Language}. Herkömmliche Abfragesprachen wären für die Anforderungen im Semantic Web zu restriktiv.
SPARQL beinhaltet deswegen den OPTIONAL Operator. Diese äu{\ss}erst wichtige Funktion ermöglicht Abfragen zu formulieren, welche das Ergebnis um gewisse Teile erweitert falls diese verfügbar sind. Dabei werden die Ergebnisse bei denen es nicht zutrifft, nicht verworfen, sondern sind auch Teil der Lösung.
Die Arbeit beschäftigt sich im Weiteren mit einem Teilproblem des Modular Query Rewriting \cite[S.~5]{CertainAnswers} . Die Auswertung einer konkreten Abfrage dieses Algorithmus ist der Einstiegspunkt in diese Arbeit.\section*{Ziel}Die derzeitige Implementierung basiert auf einem einfachen iterativen Vorgehen. Das Ziel ist die Implementierung eines Algorithmus, welcher unter Zuhilfenahme einer In-Memory Datenbank die Leistung verbessert und somit die Durchlaufzeit verringert. Bei diesen Tests kommen verschiedene In-Memory Datenbanken zum Einsatz, um für diesen speziellen Anwendungsfall eine geeignete Implementierung auszuwählen.\section*{Aufbau der Arbeit}Die Arbeit ist folgenderma{\ss}en aufgebaut. Zu Beginn wird der derzeitige Algorithmus beschrieben und analysiert. Im speziellen wird die Leistung des derzeitigen Algorithmus als Orientierungswert für den späteren Vergleich herangezogen. Des Weiteren wird die Datenstruktur der Eingabedaten erläutert, diese Daten dienen als Eingabe für den derzeitigen Algorithmus, als auch als Eingabe für den zukünftigen Algorithmus.
...
\begin{algorithm}\SetKwInOut{Input}{input}\Input{Set $results$ aus Mappings, Set $M$ aus Mappings}\BlankLine\ForEach{Mapping $m1$ aus $results$}{
  \ForEach{Mapping $m2$ aus $M$}{    \If{$m1$ kompatibel mit $m2$}{
      Erweitere $m1$ mit $m2$
    }  }}\caption{Erweiterung der Mappings}\label{alg:erw}\end{algorithm}In der dritten Phase findet die Projektion der Daten statt. Hierbei wird über alle Mappings iteriert und alle SPARQL-Variablen entfernt, welche nicht in der Projektion (\textit{ovar} Elemente) enthalten sind. 

\paragraph{Maximierung} \label{maxMapPar}
Die vierte und letzte Phase beschäftigt sich mit der Maximierung der Mappings (\cite[S.~4]{CertainAnswers} unter CERTAIN ANSWER SEMANTICS) aus \textit{results}. Alle Elemente aus \textit{results} werden in eine neue Menge \textit{maxResults} überführt. Zu Beginn ist die Menge \textit{maxResults} leer. Die Elemente aus \textit{results} werden einzeln in \textit{maxResults} eingefügt. Um ein Element zur Menge hinzuzufügen wird folgender Algorithmus \ref{alg:max} angewandt.
...
\Fn{Boolean subsums(Mapping m1, Mapping m2)}{
  \eIf{Anzahl der Elemente von m1 $\geq$ Anzahl der Elemente von m2}{
    $keys1\leftarrow$ Menge der Schlüssel von $m1$\;
    $keys2\leftarrow$ Menge der Schlüssel von $m2$\;
    \eIf{$keys1$ beinhaltet alle Elemente aus $m2$}{
      \ForEach{Schlüssel $k$ aus $keys1$}{
        $v1\leftarrow m1[k]$\;
        $v2\leftarrow m2[k]$\;        \If{$v1\neq v2$ AND $v2\neq null$}{
          \Return false\;        }
      }
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item Auswertungsergebnis siehe \ref{lst:bi15}
	\end{itemize}
	\item lubm-ex-20-17.sparql.xml
	\begin{itemize}
		\item Dateigrö{\ss}e: 24,95 MB
		\item Zeilenanzahl: 378.450
		\item Auswertungsergebnis siehe \ref{lst:bi17}
	\end{itemize}
\end{itemize}
